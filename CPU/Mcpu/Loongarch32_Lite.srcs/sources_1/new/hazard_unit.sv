...
module hazard_unit(
    ...
    // 来自解码阶段的信号
    input  wire [`REG_ADDR_BUS]  id_ra1_i,          // 解码阶段rs1地址
    input  wire [`REG_ADDR_BUS]  id_ra2_i,          // 解码阶段rs2地址（含store的rd）
    ...
    wire rs1_mem_hazard = (id_ra1_i == mem_wa_i) && mem_wreg_i && (id_ra1_i != `REG_NOP);
    wire rs1_wb_hazard  = (id_ra1_i == wb_wa_i)  && wb_wreg_i  && (id_ra1_i != `REG_NOP);

    // 数据冒险检测：rs2/rd冲突（含store的rd）
    wire rs2_exe_hazard = (id_ra2_i == exe_wa_i) && exe_wreg_i && (id_ra2_i != `REG_NOP);
    wire rs2_mem_hazard = (id_ra2_i == mem_wa_i) && mem_wreg_i && (id_ra2_i != `REG_NOP);
    wire rs2_wb_hazard  = (id_ra2_i == wb_wa_i)  && wb_wreg_i  && (id_ra2_i != `REG_NOP);

    // load-use冒险检测（PPT核心暂停场景：EXE是LOAD，且ID阶段使用其结果）
    wire load_use_hazard = (exe_alutype_i == `LOAD) && (rs1_exe_hazard || rs2_exe_hazard);

    // 数据前推选择逻辑（优先级：EXE > MEM > WB，符合PPT前推设计）
    always @(*) begin
        // rs1前推选择
    ...
    // 流水线暂停逻辑：仅load-use冒险时暂停（IF/ID保持，EXE清零）
    always @(*) begin
