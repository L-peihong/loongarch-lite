...
wire inst_or       = (opcode1 == 6'h00) && (opcode2 == 4'h00) && (opcode3 == 7'h2A);  // 修正为0x2A
wire inst_xor      = (opcode1 == 6'h00) && (opcode2 == 4'h00) && (opcode3 == 7'h2B);  // 修正为0x2B
wire inst_sllw     = (opcode1 == 6'h00) && (opcode2 == 4'h00) && (opcode3 == 7'h2E);  // sll.w: 0x2E
wire inst_addiw    = (opcode1 == 6'h00) && (opcode2 == 4'h08);  // I12-type: addi.w
wire inst_sltui    = (opcode1 == 6'h00) && (opcode2 == 4'h09);  // I12-type: sltui
wire inst_andi     = (opcode1 == 6'h00) && (opcode2 == 4'h0D);  // I12-type: andi
wire inst_ori      = (opcode1 == 6'h00) && (opcode2 == 4'h0E);  // I12-type: ori
wire inst_lu12iw   = (opcode1 == 6'h05);  // U-type: lu12i.w
wire inst_pcaddu12i= (opcode1 == 6'h07);  // U-type: pcaddu12i
wire inst_load     = (opcode1 == 6'h0A);  // load opcode1=0x0A
wire inst_store    = (opcode1 == 6'h0B);  // store opcode1=0x0B
wire inst_beq      = (opcode1 == 6'h16);  // B-type: beq
wire inst_bne      = (opcode1 == 6'h17);  // B-type: bne
wire inst_bgeu     = (opcode1 == 6'h1B);  // B-type: bgeu
wire inst_ldb      = inst_load && (opcode2 == 4'h00);  // ld.b
wire inst_ldw      = inst_load && (opcode2 == 4'h02);  // ld.w
wire inst_stb      = inst_store && (opcode2 == 4'h04);  // st.b
wire inst_stw      = inst_store && (opcode2 == 4'h06);  // st.w
...
(inst_addw || inst_or || inst_xor || inst_sllw) ? rk : `REG_NOP;  // 仅3R型指令rs2=rk
...
// 操作数2前推选择（核心修正：3R型指令也通过前推选择，而非直接用rd2）
...
// 操作数2最终选择（修正3R型指令逻辑，全部通过前推后的op2_reg）
...
op2_reg;  // 3R型指令（sllw/or/xor）使用前推后的op2_reg
